...
def validate(model_path: str, x_test_path: str, y_test_path: str) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        model_path_final = Path(tmp_dir, "model_final.joblib")
        joblib.dump(model, model_path_final)
        mlflow.log_artifact(str(model_path_final), "model_final")
        logging.warning(f"model saved as artifact in model_final/")

    try:
        run_id = mlflow.active_run().info.run_id
        model_uri = f"runs:/{run_id}/model_final"
        mlflow.register_model(model_uri, "model_registered")
